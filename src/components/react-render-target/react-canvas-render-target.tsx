...
import { getEllipseArcByStartEnd } from "../../utils/ellipse"
...
export const reactCanvasRenderTarget: ReactRenderTarget<CanvasDraw> = {
  ...
  renderGroup(children, options) {
    return (ctx, scale, rerender, width, height, transform, matrix) => {
      ...
      children.forEach((c) => {
        c(ctx, scale, rerender, width, height, transform, parentMatrix)
      })
      ctx.restore()
    }
  },
  renderRect(x, y, width, height, options) {
    return (ctx, scale, rerender, globalWidth, globalHeight, transform, matrix) => {
      ctx.save()
      ctx.beginPath()
  ...
  renderText(x, y, text, fill, fontSize, fontFamily, options) {
    return (ctx, scale, rerender, width, height, transform, matrix) => {
      ...
      renderFill(ctx, scale, rerender, width, height, transform, matrix, {
        ...options,
      ...
      if (options?.strokeColor !== undefined || options?.strokePattern || options?.strokeLinearGradient || options?.strokeRadialGradient) {
        if (options?.strokeColor !== undefined) {
          ...
          renderPatternOrGradient(ctx, scale, rerender, width, height, transform, matrix, options)
        }
  ...
  renderRay(x, y, angle, options) {
    return (ctx, scale, rerender, width, height, transform, matrix) => {
      ...
      if (!line) return
      this.renderPath([line], options)(ctx, scale, rerender, width, height, transform, matrix)
    }
  },
}
...
function renderFill(
  ...
  if (options?.clip !== undefined) {
    ctx.clip('evenodd')
    options.clip()(ctx, scale, rerender, width, height, transform, matrix)
  }
  if (options?.fillColor !== undefined || options?.fillPattern !== undefined || options?.fillLinearGradient !== undefined || options?.fillRadialGradient !== undefined) {
    if (options.fillPattern !== undefined) {
      const pattern = getPattern(ctx, options.fillPattern, scale, rerender, width, height, transform, matrix)
      if (pattern) {
...
function renderPatternOrGradient(
  ...
  if (options?.strokePattern) {
    const pattern = getPattern(ctx, options.strokePattern, scale, rerender, width, height, transform, matrix)
    if (pattern) {
    ...
    if (ref.current) {
      ...
      if (ctx) {
        ...
        for (const draw of props.draws) {
          draw(ctx, scale, rerender, props.width, props.height, props.transform)
        }
        ctx.restore()
        if (props.debug) {
