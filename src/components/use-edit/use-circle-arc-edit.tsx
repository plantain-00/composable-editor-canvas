...
import { Arc } from "../.."
import { getTwoPointsAngle, getTwoPointsDistance, Position } from "../../utils"
  ...
  const { onStart, mask } = useDragEdit<{ type: 'center' | 'start angle' | 'end angle' | 'radius' } & Arc, T>(
    ...
    (start, end) => {
      ...
      if (start.data.type === 'center') {
        ...
        const angle = getTwoPointsAngle(end, start.data) * 180 / Math.PI - start.data.startAngle
        setOffset({ x: 0, y: 0, r: 0, startAngle: angle, endAngle: 0, data: start.data.data })
      } else if (start.data.type === 'end angle') {
        const angle = getTwoPointsAngle(end, start.data) * 180 / Math.PI - start.data.endAngle
