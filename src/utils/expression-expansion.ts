...
import { Factor, FactorVariable, isLetter, isNumber } from "../components";
...
export function mathStyleExpressionToExpression(e: string) {
  ...
  for (let i = 0; i < e.length; i++) {
    ...
    if (c === ' ' && i > 0 && i < e.length - 1) {
      if ((isLetter(e[i - 1]) || isNumber(e[i - 1])) && (isLetter(e[i + 1]) || isNumber(e[i + 1]))) {
        result += '*'
        continue
      }
    }
