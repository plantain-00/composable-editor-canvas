...
export function getLinesTangentTo2GeometryLines(line1: GeometryLine, line2: GeometryLine): Tuple2<Position>[] {
  ...
  if (line1.type === 'ellipse arc') {
    ...
    if (line2.type === 'quadratic curve') {
      return getLinesTangentToEllipseArcAndQuadraticCurve(line1.curve, line2.curve)
    }
    if (line2.type === 'bezier curve') {
  ...
  if (line1.type === 'quadratic curve') {
    ...
    return getLinesTangentToQuadraticCurveAndNurbsCurve(line1.curve, line2.curve)
  }
  ...
  if (line1.type === 'bezier curve') {
    ...
    return getLinesTangentToBezierCurveAndNurbsCurve(line1.curve, line2.curve)
  }
...
export function getLinesTangentToEllipseArcAndQuadraticCurve(curve1: EllipseArc, curve2: QuadraticCurve): Tuple2<Position>[] {
  const [p1, d1, d2] = getEllipseDerivatives(curve1)
  const [p2, e1, e2] = getQuadraticCurveDerivatives(curve2)
  const f1 = (t: Vec2): Vec2 => {
    // (y1 - y2)/(x1 - x2) = y1'/x1' = y2'/x2'
...
export function getLinesTangentToEllipseArcAndBezierCurve(curve1: EllipseArc, curve2: BezierCurve): Tuple2<Position>[] {
  const [p1, d1, d2] = getEllipseDerivatives(curve1)
  const [p2, e1, e2] = getBezierCurveDerivatives(curve2)
  const f1 = (t: Vec2): Vec2 => {
    // (y1 - y2)/(x1 - x2) = y1'/x1' = y2'/x2'
...
export function getLinesTangentToEllipseArcAndNurbsCurve(curve1: EllipseArc, curve2: NurbsCurve): Tuple2<Position>[] {
  const [p1, d1, d2] = getEllipseDerivatives(curve1)
...
export function getLinesTangentToBezierCurveAndNurbsCurve(curve1: BezierCurve, curve2: NurbsCurve): Tuple2<Position>[] {
  const [p1, d1, d2] = getBezierCurveDerivatives(curve1)
  const nurbs2 = toVerbNurbsCurve(curve2)
  const f1 = (t: Vec2): Vec2 => {
    // (y1 - y2)/(x1 - x2) = y1'/x1' = y2'/x2'
...
export function getLinesTangentTo2Hyperbolas(curve1: HyperbolaSegment, curve2: HyperbolaSegment): Tuple2<Position>[] {
...
export function getLinesTangentToTwoNurbsCurves(curve1: NurbsCurve, curve2: NurbsCurve): Tuple2<Position>[] {
  ...
  return ts.filter(v => isBetween(v[0], 0, maxParam1) && isBetween(v[1], 0, maxParam2)).map(t => {
    return [fromVerbPoint(nurbs2.point(t[0])), fromVerbPoint(nurbs2.point(t[1]))]
  })
}
