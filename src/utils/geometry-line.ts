...
import { fromVerbPoint, getNurbsCurveCurvatureAtParam, getNurbsCurveDerivatives, getNurbsCurveParamAtPoint, getNurbsCurvePointAtParam, getNurbsCurveStartAndEnd, getNurbsMaxParam, getPartOfNurbsCurve, NurbsCurve, pointIsOnNurbsCurve, toVerbNurbsCurve } from "./nurbs";
...
{ type: 'hyperbola curve', curve: HyperbolaSegment } |
{ type: 'ray', line: Ray }
...
export const GeometryLine = (v: unknown, path: Path): ValidationResult => {
  ...
  if (v.type === 'hyperbola curve') return validate(v, { type: 'hyperbola curve', curve: HyperbolaSegment }, path)
  if (v.type === 'ray') return validate(v, { type: 'ray', line: Ray }, path)
  return { path: [...path, 'type'], expect: 'or', args: ['arc', 'ellipse arc', 'quadratic curve', 'bezier curve', 'nurbs curve', 'hyperbola curve', 'ray'] }
}
...
export function geometryLineInPolygon(line: GeometryLine, polygon: Position[]) {
  ...
  if (line.type === 'arc') {
    ...
    points = getHyperbolaSegmentStartAndEnd(line.curve)
...
export function getGeometryLineStartAndEnd(line: GeometryLine) {
  ...
  if (line.type === 'hyperbola curve') {
    return getHyperbolaSegmentStartAndEnd(line.curve)
  }
  if (line.type === 'ray') {
...
export function pointIsOnGeometryLine(p: Position, line: GeometryLine) {
  ...
  if (line.type === 'hyperbola curve') {
    return pointIsOnHyperbola(p, line.curve) && pointIsOnHyperbolaSegment(p, line.curve)
  }
  if (line.type === 'ray') {
...
export function getGeometryLinePointAtParam(param: number, line: GeometryLine) {
  ...
  if (line.type === 'hyperbola curve') {
    return getHyperbolaPointAtParam(line.curve, getParamAtPercent(param, line.curve.t1, line.curve.t2))
  }
  if (line.type === 'ray') {
...
export function getGeometryLineTangentRadianAtParam(param: number, line: GeometryLine): number {
  ...
  if (line.type === 'hyperbola curve') {
    return getHyperbolaTangentRadianAtParam(line.curve, getParamAtPercent(param, line.curve.t1, line.curve.t2))
  }
  if (line.type === 'ray') {
...
export function getGeometryLinePointAndTangentRadianAtParam(param: number, line: GeometryLine): { point: Position; radian: number } {
  ...
  if (line.type === 'hyperbola curve') {
    ...
    return {
      point: getHyperbolaPointAtParam(line.curve, t),
      radian: getHyperbolaTangentRadianAtParam(line.curve, t),
    }
  }
...
export function getGeometryLineCurvatureAtParam(line: GeometryLine, param: number): number {
  ...
  if (line.type === 'hyperbola curve') {
    const t = getParamAtPercent(param, line.curve.t1, line.curve.t2)
    return getHyperbolaCurvatureAtParam(line.curve, t)
  }
...
export function getGeometryLineDerivatives(line: GeometryLine): [(t: number) => Tuple2<Position>, (t: number) => Tuple3<Position>] {
  if (Array.isArray(line)) {
    // x = x0 + (x1 - x0)t
    // y = y0 + (y1 - y0)t
  ...
  if (line.type === 'ray') {
    const r = angleToRadian(line.line.angle)
    const d = line.line.reversed ? -1 : 1
    // x = x0 + cos(r) d t
    // y = y0 + sin(r) d t
    const p1 = { x: Math.cos(r) * d, y: Math.sin(r) * d }
    return [
      t => [getRayPointAtDistance(line.line, t), p1],
      t => [getRayPointAtDistance(line.line, t), p1, { x: 0, y: 0 }],
    ]
  }
  const nurbs = toVerbNurbsCurve(line.curve)
  return [
    t => {
      const derivatives = nurbs.derivatives(t)
      return [fromVerbPoint(derivatives[0]), fromVerbPoint(derivatives[1])]
    },
    t => {
      const derivatives = nurbs.derivatives(t, 2)
      return [fromVerbPoint(derivatives[0]), fromVerbPoint(derivatives[1]), fromVerbPoint(derivatives[2])]
    },
  ]
}
